...
function add_hide_node($node) {
	...
	if ($node_list != null) {
		$node_list = _strReplace($node + ",", "", $node_list);
...
function _strReplace(search, replace, str) {
...
$(document).ready(function() {
	...
	$(".nav li a i").click(function(event) {
		icon = $(this);
		
		tree_menu = $(this).parent().parent().find(".nav");
	...
	$(".left_menu  a[node='" + left_menu + "']").addClass("active");
	if ($(".left_menu a.active").length == 0) {
		url = window.location.pathname;
		$(".left_menu a[href='" + url + "']").addClass("active");
	}
